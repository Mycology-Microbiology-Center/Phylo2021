# Visualization of trees: preparation

## A. Setting up an R-Studio project
...
## C. Detour: how to deal with comments in trees
...
````rscript
getSupports <- function(bayesTree, bootTree)
{
  ## The getAllSubTrees subfunction below atomizes a tree into each individual
  ## subclades, so then we can compare and match these subclades between our 2 trees.
  ...
  bootList <- matrix('-', Nnode(bayesTree), 1)
  for (i in 1:Nnode(bayesTree))
  {
    for (j in 1:Nnode(bootTree))
    {
      ...
      if (sum(is.na(c(shared, shared2))) == 0)
      {
        ...
        supportsTable <-
          data.frame(min(as.data.frame(bayesTree[["edge"]])$V1):max(as.data.frame(bayesTree[["edge"]])$V1),
                     bootList)
...
Apply function to our trees:

````rscript
getSupports(bayesTree = bayesTree_rooted, bootTree = bootTree_rooted)
````

Next step is [plotting and annotation of the tree](https://github.com/Mycology-Microbiology-Center/Phylo2021/blob/main/docs/04.Tree_plotting.md).
